...
export default function EditSpellPage() {
  ...
  const spellNameParam = searchParams.get('name');
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
        ...
        toast({ variant: "destructive", title: "Update Failed", description: "Could not update the spell." });
    }
  }

  if (isLoading) {
    return <div className="text-center p-8">Loading spell data...</div>;
  }
  ...
  return (
    <div>
        ...
        <Card className="max-w-3xl mx-auto">
            <CardHeader>
                <CardTitle>Edit Spell</CardTitle>
                <CardDescription>
                    Update the details for {spell.name}.
                </CardDescription>
            </CardHeader>
            <CardContent>
